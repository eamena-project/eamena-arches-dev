...
def split_and_save_tables(df, sheet_name, output_dir, root_values="https://github.com/eamena-project/eamena-arches-dev/tree/main/dbs/database.eamena/data/reference_data/rm/hp/values", verbose=True):
    starts = df[df[df.columns[0]].astype(str).str.startswith('#')].index
    ends = starts[1:].tolist() + [len(df) + 1]
    sheet_name = sheet_name.strip().replace(' ', '_')
    os.makedirs(os.path.join(output_dir, sheet_name), exist_ok=True)
    
    local_markdown_table = ''  # Create a local variable for markdown lines
    ...
    for start, end in zip(starts, ends):
        ...
        level1_txt = sheet_name.replace('_', ' ')
        level1_url = f"{root_values}/{sheet_name}"
        level3_txt = table_name.replace('_', ' ')
        level3_url = f"{root_values}/{sheet_name}/{table_name}.tsv"
        level1_link = f"[{level1_txt}]({level1_url})"
        level3_link = f"[{level3_txt}]({level3_url})"
        local_markdown_table += f"| {level1_link} | {level3_link} |\n"
